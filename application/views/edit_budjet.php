      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            $in_amount = explode('@@',$data[0]->income_amount);
            $in_desc = explode('@@',$data[0]->income_desc);
            $in_mode = explode('@@',$data[0]->income_mode);
            //print_r($exp_amount);
            //$exp_status = explode(@@,$data[0]->status);
                      //$i=0; 
                    for($i=0;$i<count($in_amount);$i++) { 
                       ?>
                <tr>
                  <td><!-- <label id="sl_no" class="sl_no">1</label> --></td>
                  <td><input type="text" name="in_mode[]" class="in_mode" value="<?php echo $in_mode[$i]; ?>" ></td>
                  <td><input type="text" name="in_desc[]" class="in_desc" value="<?php echo $in_desc[$i]; ?>"></td>
                  <td><input type="text" name="in_amt[]" class="in_amt" id="in_amt" value="<?php echo $in_amount[$i]; ?>" onchange="sum_income(this.value);" ></td>
       ...
       <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            //$exp_status = explode(@@,$data[0]->status);
                      //$i=0; 
                    for($i=0;$i<count($exp_amount);$i++) { 
                       ?>
                <tr>
                  <td><!-- <label id="sl_no" class="sl_no">1</label> --></td>
                  <td><input type="text" name="exp_mode[]" class="exp_mode" value="<?php echo $exp_mode[$i]; ?>" ></td>
                  <td><input type="text" name="exp_desc[]" class="exp_desc" value="<?php echo $exp_desc[$i]; ?>"></td>
                  <td><input type="text" name="exp_amt[]" class="exp_amt" value="<?php echo $exp_amount[$i]; ?>" id="exp_amt" onchange="sum_expenses(this.value);" ></td>
                  <!-- <input type="hidden" name="id1" value="1"> -->
                  <input type="hidden" name="user_id" value="<?php echo $_SESSION['user_id'];?>">
